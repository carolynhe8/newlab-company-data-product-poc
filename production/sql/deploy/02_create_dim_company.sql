-- Deployment step 02: dim_company.
--
-- Creates or replaces the canonical company display dimension.
...
CREATE OR REPLACE VIEW `%s.%s.dim_company`
OPTIONS(
  description = 'Canonical company display dimension. Representative attributes are for convenience only; bridge_company_source remains mapping truth.'
)
AS
WITH bridge AS (
  SELECT *
