{{ config(
    materialized=var('company_mart_materialization', var('company_data_materialization', 'view')),
    schema=var('company_data_schema', 'company_data'),
    tags=['company_data_product', 'mart', 'membership']
) }}
...
membership_months AS (
  SELECT
    ...
    mm.location_name,
    mm.location_code,
    {{ normalize_membership_category('mm.membership_category') }} AS membership_category,
    mm.plan_id,
    mm.plan_name,
    ...
    b.match_confidence
  FROM {{ source('intermediate', 'int_membership_months') }} AS mm
