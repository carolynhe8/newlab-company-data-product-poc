{{ config(
    materialized=var('company_dimension_materialization', var('company_data_materialization', 'view')),
    schema=var('company_data_schema', 'company_data'),
    tags=['company_data_product', 'dimension']
) }}
...
officernd_representative AS (
  ...
  FROM bridge AS b
  JOIN {{ source('staging', 'stg_officernd_companies') }} AS o
...
bigtime_representative AS (
  ...
  FROM bridge AS b
  JOIN {{ source('staging', 'stg_bigtime_clients') }} AS bt
